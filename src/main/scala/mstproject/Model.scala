...
object Model {
  ...
  case class weightedEdge(edge: myEdge, weight: Int, removed: Boolean = false, selectedInStep: Int = -1, id: Int)

  case class vertexEdge(v: Int, edge: weightedEdge)
  case class vEdges(v: Int, edges: Iterator[weightedEdge])
  case class graphDataset(path: String, delim: String, weighted: Boolean)
}
